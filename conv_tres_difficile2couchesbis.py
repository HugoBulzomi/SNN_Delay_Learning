...
for t in range(time_data//interval):
	...
	if direction==0:
		startx = randint(4, x_input-5-4) # We leave a margin of 4 neurons on the edges of the input layer so that the whole movement can be seen by the convolution window
...
neuron_reset = NeuronReset(sampling_interval=interval-15, pops=[convolution1, convolution2])
...
sim.run(time_data, callbacks=[SimControl1, Learn1, Learn2, neuron_reset, input_clear])
